...
class GameScene: SKScene, SKPhysicsContactDelegate
{
  ...
  let invaderHit = UIImpactFeedbackGenerator(style: .light)
  
  let kShipName = "ship"
  let kShipSize = CGSize(width: 30, height: 16)
  let shipHit = UIImpactFeedbackGenerator(style: .heavy)
  ...
  func createContent()
  {
//    let invader = SKSpriteNode(imageNamed: "#imageLiteral(resourceName: "InvaderA_00.png"))
//      invader.posistion = CGPoint(x: size.width/2, y: size.height/2)
//    addChild(invader)
  ...
  func makeShip() -> SKNode
  {
    ...
    let physicsBody = SKPhysicsBody(rectangleOf: ship.frame.size)
  ...
  override func update(_ currentTime: TimeInterval)
  {
    ...
    // called before each frame is rendered, the funtion runs once per frame
  ...
  {
    guard let _ = childNode(withName: InvaderType.name) else { return true }
    guard let _ = childNode(withName: kShipName) else { return true }
